...
Built by a native Kusaal speaker. Fine-tuned on a parallel corpus assembled from scratch. Contributed to the [Masakhane](https://www.masakhane.io/) initiative for African NLP.
...
| **HuggingFace** | `PrinceAlhassan/kusaal-nllb-600M` *(coming soon)* |
...
model_dir = "path/to/kusaal-nllb-final"   # or "PrinceAlhassan/kusaal-nllb-600M" once published

tokenizer = AutoTokenizer.from_pretrained(model_dir)
model     = AutoModelForSeq2SeqLM.from_pretrained(model_dir)
model.eval()

# Re-register language codes (not saved by save_pretrained — see kusaal_lang_codes.json)
with open(f"{model_dir}/kusaal_lang_codes.json") as f:
...
| `kusaal_lang_codes.json` | Language → token ID map (not saved by HuggingFace standard) |
| `generation_config.json` | Default generation settings (num_beams=4, max_new_tokens=256) |
| `config.json` | Model architecture config |

> **Note on `kusaal_lang_codes.json`:** NLLB uses integer token IDs to identify languages at inference time (`forced_bos_token_id`). HuggingFace's `save_pretrained()` saves the token string in `added_tokens.json` but does not persist the `lang_code_to_id` dictionary. This file ensures any loader can correctly configure the model without guessing.
...
- **Tokenization:** NLLB's SentencePiece tokenizer was built without Kusaal data. Kusaal words are over-segmented into small fragments, which limits the model's ability to learn word-level patterns.
- **Dataset size:** 20,212 pairs is small for MT. High-resource pairs use hundreds of millions. Generalisation to unseen vocabulary is limited.
- **No native speaker evaluation yet:** BLEU scores measure n-gram overlap with reference translations. The only true quality test is assessment by fluent Kusaal speakers.
...
If you work in African NLP, speak Kusaal, or want to contribute data, open an issue or reach out directly.
...
Built by **Prince Nasamu Alhassan** — native Kusaal speaker, Mathematical Science with Computer Science student at the University of Ghana, Legon. Contributor to the Masakhane pan-African NLP initiative.

- GitHub: [@NasamuAlhassan](https://github.com/NasamuAlhassan)
- HuggingFace: [PrinceAlhassan](https://huggingface.co/PrinceAlhassan)
